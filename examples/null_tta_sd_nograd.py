...
# Prompt list used for evaluation
prompt_list = [
...
# Annealing control: used as (1 + tampering_coef)**i
...
def decode_images(pipe: DiffusionPipeline, latents: torch.Tensor) -> torch.Tensor:
    """
    Decode latents into images without gradient tracking.
    """
...
def decode_images_grad(pipe: DiffusionPipeline, latents: torch.Tensor) -> torch.Tensor:
    """
    Decode latents into images while preserving gradients.
    """
    latents = (latents / pipe.vae.config.scaling_factor).to(pipe.vae.dtype)
    imgs = pipe.vae.decode(latents).sample
    imgs = ((imgs / 2) + 0.5).clamp(0, 1)
...
def tweedie_x0_from_eps(x_t: torch.Tensor, eps: torch.Tensor, alpha_t: torch.Tensor) -> torch.Tensor:
    """
    Estimate x0 from x_t and predicted noise eps using Tweedie's formula.
    Handles broadcasting for batch shapes.
    """
...
# --- JPEG compressibility reward (no-grad target) ---
def jpeg_compressibility(inference_dtype=None, device=None):
    """
    JPEG compressibility proxy: returns JPEG file sizes (kB) at quality=95.
    """
    import numpy as np
    ...
    def loss_fn(im_pix_un, prompts):
        ...
        sizes = [buffer.tell() / 1000 for buffer in buffers]  # kB
...
def get_reward_fn(target_name, force_load=False):
    """
    PickScore reward model (default optimisation target).
    Falls back to a mock reward if unavailable.
    """
...
def get_aesthetic_fn(target_name, force_load=False):
    """
    Aesthetic score model (used for logging/evaluation).
    Falls back to a mock function returning NaN if unavailable.
    """
...
def get_hps_fn(target_name, force_load=False):
    """
    HPSv2 score model (used for logging/evaluation).
    Falls back to a mock function if unavailable.
    """
...
def get_imagereward_fn(target_name, force_load=False):
    """
    ImageReward model (used for logging/evaluation).
    Falls back to a mock function returning NaN if unavailable.
    """
...
def get_jpeg_reward_fn():
    """
    No-gradient target based on JPEG compressibility.
    jpeg_compressibility returns file size (kB), so define reward = -size
    (i.e., smaller JPEG size is higher reward).
    """
    jpeg_loss = jpeg_compressibility(inference_dtype=torch.float32, device=device)

    class JPEGReward:
        def __call__(self, images, prompts):
            sizes = jpeg_loss(images, prompts)  # (N,)
            return -sizes  # smaller size => higher reward

        def eval(self):
            pass

    print("Using JPEG compressibility reward (reward = -JPEG_size_kB).")
    return JPEGReward()


def save_image_tensor(img: torch.Tensor, path: str):
    """
    Save a single image tensor (CHW or NCHW) to disk as PNG.
    """
    if img.ndim == 4:
        img = img[0]
...
def make_init_latents(pipe: DiffusionPipeline, h: int, w: int, batch_size: int, gen: torch.Generator):
    """
    Create initial Gaussian latents z_T for Stable Diffusion (latent space is 1/8 resolution).
    """
    lat_h, lat_w = h // 8, w // 8
...
# =========================
# Core optimizer with ZO Null-TTA
# =========================
class CFGOptWithBeamSearch:
    def __init__(
        ...
        # Zeroth-order gradient hyperparameters (DNO-style)
        self.zo_mu = 0.02      # perturbation scale μ
        self.zo_K = 4          # number of ZO samples K

        # Freeze all model parameters (we only optimise the unconditional embedding)
    ...
    def _encode_text_pair(self, prompt: str, negative_prompt: str):
        """
        Encode (negative_prompt, prompt) into (uncond_emb, cond_emb).
        """
    ...
    def optimize(
        ...
        prompt: str,
        negative_prompt: str,
        reward_fn,  # target reward function (e.g., JPEGReward)
        ...
        # Null-text parameter to be optimised (ZO target)
        un_master = un_orig.detach().float().clone()
        un_master.requires_grad_(True)
        ...
        with torch.no_grad():
            print(f"v15+ZO: Performing initial selection from {K} noise particles at t={timesteps[0]}...")
            try:
                t_start = timesteps[0]
                ...
                print(f"v15+ZO: Initial particle {best_idx_start} selected (Score: {scores_start[best_idx_start]:.4f}).")
        ...
        for i, t in enumerate(
            tqdm(
                timesteps,
                desc="Optimizing with Particle Filter (v15 + ZO Null-TTA, NO_HIST_REG)",
                leave=False,
            )
        ):
            t_int = int(t)
            ...
            # Baseline eps_uncond used for the KL-style regularisation term
            with torch.no_grad():
            ...
            # Determine number of inner optimisation steps using the tampering-based schedule
            ...
            # ----- Objective evaluator: J(u) = λα * R - tamp * (regularisation terms) -----
            def eval_objective(un_cond: torch.Tensor):
                """
                Evaluate the objective at a given null-text embedding.
                un_cond: (1, L, D) null-text embedding
                return: (objective_scalar, reward_scalar_float)
                """
                with torch.no_grad():
                    un_cur = un_cond.to(self.pipe.unet.dtype)

                    latent_model_input = self.pipe.scheduler.scale_model_input(x_t_particle, t_int)
                    t_tensor_input = t_tensor

                    if do_classifier_free_guidance:
                        latent_model_input_reward_batch = torch.cat([latent_model_input] * 2)
                        combined_embeds_reward_batch = torch.cat([un_cur, cond_emb])
                        t_tensor_unet_input = t_tensor_input.repeat(2)
                    else:
                        latent_model_input_reward_batch = latent_model_input
                        combined_embeds_reward_batch = cond_emb
                        t_tensor_unet_input = t_tensor_input
                    ...
                    eps_loss_t = lambda_t_coeff_batch * per_sample_mse_sum[0]

                    emb_loss = F.mse_loss(un_cond, un_orig.float(), reduction='sum')

                    loss_t_obj = (
                        lambda_reward * reward_t
                        - (tamp_t * lambda_reg_scaled * eps_loss_t)
                        - (tamp_t * sig_coeff * emb_loss)
                    )

                return loss_t_obj, float(reward_t.item())

            # ----- Inner optimisation: zeroth-order (NES-style) update on un_master -----
            for inner_idx in range(current_inner_steps_int):
                opt.zero_grad(set_to_none=True)

                J0, reward0 = eval_objective(un_master)
                last_reward_value = reward0

                g_hat = torch.zeros_like(un_master)
                for k_zo in range(self.zo_K):
                    v = torch.randn_like(un_master)
                    Jk, _ = eval_objective(un_master + self.zo_mu * v)
                    g_hat += (Jk - J0) * v

                g_hat /= (self.zo_K * self.zo_mu + 1e-8)

                # Adam performs gradient descent, so set grad = -∇J to perform ascent on J
                un_master.grad = -g_hat
                opt.step()

            if last_reward_value is not None:
                print(
                    f"[Step {i + 1}/{self.T}] Approx target reward (ZO): {last_reward_value:.4f}"
                )

            # ---------- Particle branching / greedy selection ----------
            with torch.no_grad():
...
def run_single_experiment(
    ...
    print(
        f"\n--- Starting Particle Filter Experiment [v15+ZO + NO_HIST_REG]"
    ...
    print(
        ...
        f" K={num_particles} samples/step, batch_p={batch_p}, NO_HIST_REG, ZO Null-TTA)"
        f" (Opt Target: {target_reward_name}) ---"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run Particle Filter Optimization (No Historical Regulation, ZO Null-TTA)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="jpeg",
        choices=["pickscore", "aesthetic", "hpsv2", "jpeg"],
        help="Which reward to optimize: pickscore, aesthetic, hpsv2, jpeg",
    )
    ...
    elif args.target_reward == "jpeg":
        target_reward_fn = jpeg_fn
    else:
        target_reward_fn = pickscore_fn
    ...
    print(
        ...
        f" (v15 + ZO Null-TTA + Particle Filter + NO_HIST_REG)"
        f" (Opt Target: {target_reward_name}) ========="
    )
    print("=" * 60)
    ...
    for a_val, b_val, g_val in hp_triples:
        ...
        with open(csv_path, "w", newline="") as csvfile:
            ...
            writer.writerow(
                [
                    ...
                    "baseline_ir",
                    "optimized_ir",
                    "baseline_jpeg",     # JPEG reward (= -size_kB)
                    "optimized_jpeg",
                ]
            )
            ...
            for prompt_idx, current_prompt in enumerate(prompt_list):
                ...
                # Measure JPEG reward (reward = -size_kB)
                with torch.no_grad():
                    jpeg_base_tensor = jpeg_fn(base_img.unsqueeze(0), [current_prompt])
                    jpeg_opt_tensor = jpeg_fn(opt_img.unsqueeze(0), [current_prompt])
                    base_score_jpeg = float(jpeg_base_tensor[0].item())
                    best_opt_jpeg_score = float(jpeg_opt_tensor[0].item())
                ...
                current_pair_prompt_results.append(
                    {
                        ...
                        "base_jpeg": base_score_jpeg,
                        "opt_jpeg": best_opt_jpeg_score,
                        "prompt": current_prompt,
                    }
                )
            ...
            avg_base_jpeg = safe_mean(
                [r["base_jpeg"] for r in current_pair_prompt_results]
            )
            avg_opt_jpeg = safe_mean(
                [r["opt_jpeg"] for r in current_pair_prompt_results]
            )
            ...
            if args.target_reward == "pickscore":
                ...
                # Even when optimizing JPEG, keep the summary sorting metric as PickScore by default
                avg_improvement_target = avg_opt_pick - avg_base_pick
                target_base_avg = avg_base_pick
                target_opt_avg = avg_opt_pick
            else:
        ...
        print(
            f"*** (JPEGReward=-size) Base: {avg_base_jpeg:.4f}, Opt: {avg_opt_jpeg:.4f}"
        )
        print(
        ...
        all_avg_results.append(
            {
                ...
                "min_inner_steps": min_inner_steps,
                "max_inner_steps": max_inner_steps,
                "method": f"particle_filter_{num_particles}_v15_ZO_NULLTTA_NO_HIST_REG_avg (Opt: {target_reward_name})",
    ...
    print(
        ...
        f"K={num_particles} v15_ZO_NULLTTA_NO_HIST_REG "
    ...
    summary_path = os.path.join(
        base_log_dir_root,
        f"hyperparam_summary_PROMPT_AVG_v15_ZO_NULLTTA_NO_HIST_REG_target-{args.target_reward}.csv",
    )
    ...
    print(
        f"\nComprehensive average summary (v15 + ZO Null-TTA + NO_HIST_REG,"
        f" target={target_reward_name}) saved/appended to: {summary_path}"
    )
    print("-> You can use this file and per-HP results_*.csv to select Pareto points.")
